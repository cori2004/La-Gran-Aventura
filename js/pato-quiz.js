...
const phrases=['¡Cuac! Lee las cuatro opciones antes de elegir.','Una pregunta a la vez. ¡Vamos con calma!','Mi pico no conoce la respuesta… ¡pero confío en ti!','No soy un pato cualquiera: soy tu compañero de quiz.','Las letras son grandes, ¡mis patitas son pequeñas!','Si fallas una, respira. La siguiente es otra oportunidad.','Estoy hecho de cajitas y lleno de ganas de aprender.','Lee primero la pregunta. Luego elige tu respuesta.','¡Cuac! Al terminar puedes guardar tu calificación.','Preguntas diferentes, las mismas ganas de aprender.'];let phrase=0;let last=0;let raf=0;let pointer={x:-999,y:-999};
...
document.addEventListener('pointermove',e=>{if(e.pointerType!=='mouse')return;pointer={x:e.clientX,y:e.clientY};if(raf)return;raf=requestAnimationFrame(now=>{raf=0;if(paused||document.hidden||document.querySelector('dialog[open]')||now-last<600)return;const r=sprite.getBoundingClientRect();const dx=r.left+r.width/2-pointer.x,dy=r.top+r.height/2-pointer.y;const d=Math.hypot(dx,dy);if(d>150)return;last=now;let x=zone.offsetLeft+dx/(d||1)*220,y=zone.offsetTop+dy/(d||1)*160;if(x<8||x>innerWidth-zone.offsetWidth-8)x=pointer.x<innerWidth/2?innerWidth-zone.offsetWidth-15:10;if(y<175||y>innerHeight-zone.offsetHeight-8)y=pointer.y<innerHeight/2?innerHeight-zone.offsetHeight-15:175;place(x,y);say('¡Cuac! Me hago a un lado para que puedas responder.');});},{passive:true});
...
window.addEventListener('quiz-feedback',e=>say(e.detail==='correct'?'¡Cuac, bien pensado! Sigue así.':e.detail==='wrong'?'¡Ánimo! La siguiente es una nueva oportunidad.':'Respira. ¡Vamos a la siguiente!'));
})();